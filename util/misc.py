...
def get_n_parts(points, bottoms, long_edge_1, long_edge_2, unit_disk):
    edge_length_1 = [norm2(points[e1] - points[e2]) for e1, e2 in long_edge_1]
    edge_length_2 = [norm2(points[e1] - points[e2]) for e1, e2 in long_edge_2]
    total_length_1 = sum(edge_length_1)
    total_length_2 = sum(edge_length_2)
    total_length = (total_length_1 + total_length_2) / 2
    width = []
    for b in bottoms:
        width.append(norm2(points[b[0]] - points[b[1]]))
    width = np.asarray(width)
    width_length = width.mean()
    if(width_length == 0):
        width_length = total_length
    try:
        res = unit_disk * (int(total_length / width_length) + 1)
    except:
        res = unit_disk
